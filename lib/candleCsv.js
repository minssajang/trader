...
// "2026.01.02" + "09:00:00" 또는 "2026-01-02 09:00:00" 등을 UTC 초단위 타임스탬프로
...
export function parseCandleCsv(text) {
  ...
  return {
    rows,
    dateFrom: new Date(rows[0].time * 1000).toISOString().slice(0, 10),
    dateTo: new Date(rows[rows.length - 1].time * 1000).toISOString().slice(0, 10),
  }
}
