...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { data, error } = await supabase
      .from('keyword_picks')
      .upsert({ tool_id, keyword, pc, mobile, total, competition, hint, memo }, { onConflict: 'tool_id,keyword' })
      .select().single()
    if (error) return res.status(500).json({ error: error.message })
    return res.status(200).json(data)
  }
  ...
  if (req.method === 'PATCH') {
    ...
    const { data, error } = await supabase.from('keyword_picks').update(patch).eq('tool_id', tool_id).eq('keyword', keyword).select().single()
    if (error) return res.status(500).json({ error: error.message })
    return res.status(200).json(data)
  }
  ...
  if (req.method === 'DELETE') {
    ...
    if (!tool_id || !keyword) return res.status(400).json({ error: 'tool_id, keyword 필요' })
    const { error } = await supabase.from('keyword_picks').delete().eq('tool_id', tool_id).eq('keyword', keyword)
    if (error) return res.status(500).json({ error: error.message })
