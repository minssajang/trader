/**
 * /api/admin/doc-batch
 * 전체 hint에 걸쳐 doc_count=null인 키워드를 우선순위에 따라 일괄 수집 (fresh-season 원본 그대로 이식)
 *
...
async function getTodayUsed() {
  try {
    const { data } = await supabase.from('doc_batch_log').select('used').eq('date', todayStr()).single()
    return data?.used || 0
  } catch { return 0 }
}
...
async function getNullSummary(priority = 'search_volume') {
  const { data: rows, error } = await supabase
    .from('keyword_stats').select('hint, keyword, total').is('doc_count', null).order('total', { ascending: false })
  ...
  const { data: allRows } = await supabase.from('keyword_stats').select('hint')
  const totalMap = {}
  for (const r of (allRows || [])) totalMap[r.hint] = (totalMap[r.hint] || 0) + 1

  const summary = Object.values(map).map(h => ({ ...h, totalCount: totalMap[h.hint] || h.nullCount, nullRatio: h.nullCount / (totalMap[h.hint] || 1) }))

  if (priority === 'null_ratio') summary.sort((a, b) => b.nullRatio - a.nullRatio)
  else if (priority === 'hint_order') { /* DB 순서 그대로 */ }
  else summary.sort((a, b) => b.topTotal - a.topTotal)
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      const { count: totalNull } = await supabase.from('keyword_stats').select('*', { count: 'exact', head: true }).is('doc_count', null)
      const summary = await getNullSummary('search_volume')
      return res.status(200).json({ daily_limit: DAILY_LIMIT, today_used: todayUsed, remaining, total_null: totalNull || 0, hints: summary })
    } catch (err) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      if (remaining <= 0) {
        return res.status(200).json({ done: true, reason: 'daily_limit', today_used: todayUsed, daily_limit: DAILY_LIMIT, remaining: 0, filled: 0, total_null: null })
      }

      const actualChunk = Math.min(chunkSize, remaining)
      const { data: nullRows, error } = await supabase
        .from('keyword_stats').select('hint, keyword, total').is('doc_count', null).order('total', { ascending: false }).limit(actualChunk)
      if (error) throw new Error(error.message)
      if (!nullRows || nullRows.length === 0) {
        return res.status(200).json({ done: true, reason: 'all_filled', today_used: todayUsed, remaining, filled: 0, total_null: 0 })
      }

      const keywords = nullRows.map(r => r.keyword)
      const counts = await fetchBatch(keywords, 8)
      const updates = nullRows.map((r, i) => ({ hint: r.hint, keyword: r.keyword, doc_count: counts[i] })).filter(u => u.doc_count != null)
      if (updates.length > 0) await supabase.from('keyword_stats').upsert(updates, { onConflict: 'hint,keyword', ignoreDuplicates: false })
      ...
      return res.status(200).json({
        ...
        today_used: newUsed, daily_limit: DAILY_LIMIT, remaining: newRemaining,
        processed: nullRows.length, filled: updates.length, failed: nullRows.length - updates.length, still_null: stillNull,
      })
    } catch (err) {
      return res.status(500).json({ error: err.message })
    }
