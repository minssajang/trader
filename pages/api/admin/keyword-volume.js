/**
 * /api/admin/keyword-volume
 * 네이버 검색광고 API — 연관 키워드 검색량 + 네이버 블로그 문서수 조회 (fresh-season 원본 그대로 이식)
 *
...
function todayStr() { return nowKST().slice(0, 10) }

async function addTodayUsed(count) {
  if (!count || count <= 0) return
  try {
    const today = todayStr()
    const { data: existing } = await supabase.from('doc_batch_log').select('used').eq('date', today).single()
    const newUsed = (existing?.used || 0) + count
    await supabase.from('doc_batch_log').upsert({ date: today, used: newUsed, updated_at: nowKST() }, { onConflict: 'date' })
  } catch (e) { console.error('doc_batch_log 기록 오류:', e.message) }
}
...
export default async function handler(req, res) {
  ...
  const { keyword, mode = 'all', chunk: chunkParam = '50', offset: offsetParam = '0' } = req.query
  ...
  if (mode === 'doc_only') {
    try {
      const { data: nullRows, error: fetchErr } = await supabase
        .from('keyword_stats').select('keyword', { count: 'exact' })
        .eq('hint', keyword).is('doc_count', null).order('keyword')
        .range(offset, offset + chunkSize - 1)
      if (fetchErr) throw new Error(fetchErr.message)

      const { count: totalNull } = await supabase
        .from('keyword_stats').select('*', { count: 'exact', head: true })
        .eq('hint', keyword).is('doc_count', null)

      if (!nullRows || nullRows.length === 0) {
        return res.status(200).json({ keyword, mode, chunk: chunkSize, offset, total_null: totalNull || 0, filled: 0, still_null: totalNull || 0, done: true, message: '모든 문서수 수집 완료' })
      }

      const nullKeywords = nullRows.map(r => r.keyword)
      const counts = await fetchDocCountBatch(nullKeywords, 10)
      const updates = nullKeywords.map((kw, i) => ({ keyword: kw, doc_count: counts[i] })).filter(u => u.doc_count != null)

      if (updates.length > 0) {
        await supabase.from('keyword_stats').upsert(
      ...
      return res.status(200).json({ keyword, mode, chunk: chunkSize, offset, processed: nullKeywords.length, filled: updates.length, failed: nullKeywords.length - updates.length, still_null: stillNull, done })
    } catch (err) {
  ...
  if (!apiKey || !secretKey || !customerId) return res.status(500).json({ error: '네이버 검색광고 API 환경변수 누락 (NAVER_AD_API_KEY/NAVER_AD_SECRET_KEY/NAVER_AD_CUSTOMER_ID)' })
  ...
  try {
    const response = await fetch(requestUrl, {
      method,
      headers: { 'X-Timestamp': timestamp, 'X-API-KEY': apiKey, 'X-Customer': customerId, 'X-Signature': signature, 'Content-Type': 'application/json' },
    })
    ...
    const parsed = list.map(item => {
      ...
      return { hint: keyword, keyword: item.relKeyword, pc, mobile, total: pc + mobile, competition: item.compIdx || '-' }
    }).sort((a, b) => b.total - a.total)

    const keywordList = parsed.map(i => i.keyword)
    const { data: existing } = await supabase
      .from('keyword_stats').select('keyword, doc_count').eq('hint', keyword).in('keyword', keywordList)
    const existingMap = {}
    ...
    const withDocCount = parsed.map(item => ({
      ...item,
      doc_count: docCountMap[item.keyword] !== undefined ? docCountMap[item.keyword] : (existingMap[item.keyword] ?? null),
      created_at: nowIso,
    }))

    if (withDocCount.length > 0) {
      const { error: dbError } = await supabase.from('keyword_stats').upsert(withDocCount, { onConflict: 'hint,keyword' })
      if (dbError) console.error('Supabase 저장 오류:', dbError.message)
    }

    const results = withDocCount.map(({ hint, ...rest }) => rest)
    if (mode === 'all' && fetchedCount > 0) await addTodayUsed(fetchedCount)

    res.setHeader('Cache-Control', 'no-store')
    return res.status(200).json({
      keyword, mode, total_found: list.length, saved: withDocCount.length,
      doc_count_fetched: fetchedCount, doc_count_skipped: parsed.length - fetchedCount, results,
    })
  } catch (err) {
    return res.status(500).json({ error: err.message })
  }
