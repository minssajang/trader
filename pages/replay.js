      ...
      <div className="bt-page" style={{ minHeight: '100vh', background: '#0f1115', color: '#e8eaed', fontFamily: '-apple-system, "Segoe UI", "Malgun Gothic", sans-serif' }}>
        ...
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 28px', borderBottom: '1px solid #2a2e38' }}>
          ...
          <nav style={{ display: 'flex', gap: 6 }}>
            ...
            <Link href="/live" style={{ padding: '7px 14px', borderRadius: 8, fontSize: 13, fontWeight: 700, color: '#9aa0ab', border: '1px solid #2a2e38', textDecoration: 'none' }}>🔴 라이브</Link>
          </nav>
        </header>
            ...
            <div style={{ flex: 1, minWidth: 280, position: 'sticky', top: 20, maxHeight: 'calc(100vh - 40px)', overflowY: 'auto', overflowX: 'hidden' }}>
              ...
              <div style={{ background: '#171a21', border: '1px solid #2a2e38', borderRadius: 14, padding: 16, position: 'relative' }}>
                ...
                {showSemiAutoSignalOnChart && (() => {
                  ...
                  // 예약 카드의 "화면에 보이는 순서"를 그대로 따른다 - 5,6번(주가 vs H1)과 7,8번(5Bol)의
                  // 자리를 서로 맞바꿈(사용자 요청): 1=1번(내부row4, 하락추세)/2=2번(내부row3, 상승추세)/
                  // 3=3번(HMA300 하락중)/4=4번(HMA300 상승중)/5=5번(내부row6, 주가<H1)/6=6번(내부row5,
                  // 주가>H1)/7=7번(내부row1, 5Bol 상단)/8=8번(내부row1.1, 5Bol 하단)/9=9번(스토 데드)/
                  // 10=10번(스토 골든). 1↔2, 3↔4, 5↔6, 9↔10은 각각 같은 비교식의 반대 방향이라 절대
                  // 동시에 못 뜬다.
                  ...
                  if (price != null && h1 != null && prevH1 != null && price < h1 && h1 < prevH1) shortRows.push(5) // 5번: 주가<H1, H1 하락중
                  if (price != null && h1 != null && prevH1 != null && price > h1 && h1 > prevH1) longRows.push(6) // 6번: 주가>H1, H1 상승중
                  if (row1Armed === 'above') shortRows.push(7) // 7번(매도 전용): 5Bol 상단 밖(row1Armed 진입 전)
                  if (row1Armed === 'below') longRows.push(8) // 8번(매수 전용): 5Bol 하단 밖(row1Armed 진입 전)
