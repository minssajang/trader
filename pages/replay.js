...
export default function ReplayChart() {
  ...
  const renderTwReservationTab = (which) => {
    ...
    // 흘려서 좁은 폭(표시등 컬럼과 같은 44px)에 들어가게 한다. disabled는 A카드/B카드에 C,D,E,F,G,H가
    // 추가되면서(사용자 요청, 셀/바이 버튼 공용) 생김 - 그룹 중 아무 것도 체크 안 돼있으면 눌러도
    // 대상이 없으니 비활성화로 표시.
            ...
            <div style={{ flex: 1, minWidth: 280, position: 'sticky', top: 20, maxHeight: 'calc(100vh - 40px)', overflowY: 'auto', overflowX: 'hidden' }}>
              ...
              <div style={{ background: '#171a21', border: '1px solid #2a2e38', borderRadius: 14, padding: 16, position: 'relative' }}>
                ...
                {showSemiAutoSignalOnChart && (() => {
                  ...
                  // 4=4번(HMA300 상승중)/A=A(내부row6, 주가<H1)/B=B(내부row5, 주가>H1)/5=5번(내부row1,
                  // 5Bol 상단)/6=6번(내부row1.1, 5Bol 하단)/7=7번(스토 데드)/8=8번(스토 골든).
                  // 1↔2, 3↔4, A↔B, 7↔8은 각각 같은 비교식의 반대 방향이라 절대 동시에 못 뜬다.
                  ...
                  if (price != null && h1 != null && prevH1 != null && price < h1 && h1 < prevH1) shortRows.push('A') // A: 주가<H1, H1 하락중
                  if (price != null && h1 != null && prevH1 != null && price > h1 && h1 > prevH1) longRows.push('B') // B: 주가>H1, H1 상승중
